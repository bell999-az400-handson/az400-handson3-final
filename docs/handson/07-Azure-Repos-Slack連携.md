...
1. Webブラウザで以下を開く：
   ...
   ```
2. ページ内の **「Create your Slack app」** または **「Incoming Webhooks」** のリンクをクリック

**方法2: App Directoryから**
1. **Slack ワークスペース**にログイン（ブラウザ）
2. **App Directory** を開く：
   ```
   https://<your-workspace>.slack.com/apps
   ```
   例: `https://az400-handson3.slack.com/apps`

#### 1.2 Incoming WebHooks をインストール

⚠️ **注意**: この操作は Slack の **App Directory（Webブラウザ）** で行います。Slack デスクトップアプリのチャンネルメニューからは設定できません。

1. **Webブラウザ**で以下のURLにアクセス：
   ```
   https://api.slack.com/messaging/webhooks
   ```
   または、ワークスペースのApp Directoryを開く：
   ```
   https://<your-workspace>.slack.com/apps
   ```

2. 検索ボックスに **「Incoming WebHooks」** と入力

3. **「Incoming WebHooks」** アプリをクリック

4. **「Add to Slack」** ボタンをクリック

#### 1.3 Webhook の設定

1. **通知先のチャンネル**を選択：
   - ドロップダウンから既存チャンネルを選択（例: `#all-az400-handson3`）
   - または「create a new channel」で新しいチャンネルを作成

2. **「Add Incoming WebHooks integration」** ボタンをクリック
   （このボタンはApp Directoryのページに表示されます）
...
#### 💡 代替方法: Azure Repos for Slack アプリを使用（より簡単）

Slackに **「Azure Repos」** アプリが既にインストールされている場合、Webhookより簡単に設定できます：
...
**設定方法**:
1. Slack で **「Azure Repos」** アプリを開く
2. **「Add to Slack」** または **「Authorize」** をクリック
3. Azure DevOps Organization にサインイン
4. 通知したいリポジトリとチャンネルを選択
5. 完了

⚠️ ただし、この演習では **Service Hooks の理解** のため、Incoming Webhook 方式を推奨します。
